 ...
 * @param {Object} cfg - selectors passed from HTML
 */
window.coa.initListPage = function initListPage(cfg){
  ...
  let state = { page:1, pageSize:10, totalPages:1, rows:[], rowsAll:[] };
  ...
  async function load(){
    try{
      ...
      const filtered = all.filter(r => {
        ...
        // NOTE: date range filter omitted here until API returns dates suitable for compare
        return true;
      });
  ...
  function rowHtml(r){
    ...
    return `
      <tr class="table__row" data-acct="${r.number}" style="cursor:pointer">
        ...
        <td class="text-right">
          <button class="btn btn--secondary" onclick="event.stopPropagation(); window.location.href='./coa-form.html?id=${r.id}'">Edit</button>
        </td>
      </tr>`;
  }
